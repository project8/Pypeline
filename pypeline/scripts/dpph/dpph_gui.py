...
class measure_dpph:
    ...
    def BuildGui(self):
        ...
        # input power
        Label(self.toplevel, text='Input Power').grid(row=1, column=0, sticky='ew')
        Entry(self.toplevel, textvariable=self.powerVar
              ).grid(row=1, column=1, sticky='ew')
        Label(self.toplevel, text='[dBm]', justify='left').grid(row=1, column=2)
        ...
        Label(linear_frame, text='Sigma limit').grid(row=0, column=0, sticky='ew')
        Entry(linear_frame, textvariable=self.nsigmavar
              ).grid(row=0, column=1, sticky='ew')
        Label(linear_frame, text='(a real)', justify='left').grid(row=0, column=2)
        ...
        # actions
        Button(linear_frame, text="Start Scan", command=lambda: self.DoRun('linear_fit')
               ).grid(row=2, column=0)
        ...
        # actions
        Button(fft_frame, text="Start Scan", command=lambda: self.DoRun('fft_filter')
               ).grid(row=2, column=0)
