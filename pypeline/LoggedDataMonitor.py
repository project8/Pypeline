...
class LoggedDataMonitor:
    ...
    def __init__(self, couch_url="http://127.0.0.1:5984"):
        ...
        thedata = self._logged_data.view('log_access/all_logged_data', startkey=start_time.strftime(
            "%Y-%m-%d %H:%M:%S"), endkey=myendkey)
    ...
    def UpdateData(self, feed='longpoll'):
        '''
                ...
                has changed
        '''
        changes = self._logged_data.changes(
            since=self.last_seq, feed=feed)
    ...
    def EatDocument(self, doc):
        ...
        if self.times.get(sensor) is None:
            self.times[sensor] = []
            self.values[sensor] = []
