...
hguess = round(abs(float(drip.Get(
    'hall_probe_voltage').Wait()['final'][1:16])) * hallconst, 3)
print "Hall Probe Estimated Field: " + str(hguess) + " T"

# Estimate field with coil constant
cguess = round(float(raw_input(
    "Enter magnet current in Amps: ")) * coilconst + coiloffset, 3)
print "Coil constant estimated field: " + str(cguess) + " T"

# Check for reasonable agreement
# if int(1000*abs(hguess-cguess)) > 10:
# sys.exit('Hall Probe and coil constant do not agree. Try again or check
# calibration.')
...
while runnum < runlim:
    ...
    drip.Set('hf_sweep_start', target - (
        scanwin * (runlim - 1) + 100) / 2 + scanwin * runnum)
    drip.Set('hf_sweep_stop', target - (scanwin * (
        runlim - 1) + 100) / 2 + scanwin * runnum + 100)
    drip.Set('hf_sweep_time', 10)
    drip.Set('lo_cw_freq', target - 24500 - (
        scanwin * (runlim - 1) + 100) / 2 + scanwin * runnum)
    sigrun = eval(eval(repr(drip.CreatePowerSpectrum(drip.Run(
        filename=tempf, duration=scantime).Wait(), sp="sweepline").Wait()))['result'])
    sigtp = sigrun['data']
    tf = list(np.linspace(target - (scanwin * (runlim - 1) + 100) / 2 + scanwin * runnum, sigrun[
              'sampling_rate'] / 2 + target - (scanwin * (runlim - 1) + 100) / 2 + scanwin * runnum, len(sigtp)))
    sigpow = sigpow + sigtp[lb:ub]
    freq = freq + tf[lb:ub]
    drip.Set('pancake_coil_current', '4A')
    bgrun = eval(eval(repr(drip.CreatePowerSpectrum(drip.Run(
        filename=tempf, duration=scantime).Wait(), sp="sweepline").Wait()))['result'])
    bgtp = bgrun['data']
...
plt.savefig(
    '/Users/Micah/project8/pypeline/scripts/dpph_images/' + fname + '.jpg')
plt.show()
