...
hguess = round(abs(float(repr(drip.Get('hall_probe_voltage'))[1:16]))*hallconst,3)
...
while runnum < runlim:
    ...
    sigrun = eval(repr(drip.Run(subprocess = "sweepline", filename = tempf, duration = scantime))[1:-1])
    sigtp = sigrun['data']
    ...
    bgrun = eval(repr(drip.Run(subprocess = "sweepline", filename = tempf, duration = scantime))[1:-1])
    bgtp = bgrun['data']
